...
//		}
//		if(!cols.contains(Constants.TCID)
//				|| !cols.contains("USID")
...
//				|| !cols.contains("Status")){
		if(!cols.contains(Constants.TCID)
			...
			APP_LOGS.error("Defined Columns are "+TestUtil.getListAsString(cols));
...
//		if(!row.getCell(0).getStringCellValue().trim().equalsIgnoreCase(Constants.TCID)
//				|| !row.getCell(1).getStringCellValue().trim().equalsIgnoreCase("USID")
//				|| !row.getCell(2).getStringCellValue().trim().equalsIgnoreCase("Description")
//				|| !row.getCell(3).getStringCellValue().trim().equalsIgnoreCase(Constants.RUNMODE)){
//			APP_LOGS.error("Column names not defined properly in Test Cases Sheet.. Please check if there any typos");
//			return false;
//		}
...
//		}
		...
		if(!cols.contains(Constants.TCID)
				|| !cols.contains("Description")
				|| !cols.contains(Constants.KEYWORD)
				|| !cols.contains(Constants.OBJECT)
				|| !cols.contains(Constants.DATA)
...
//		if(!row.getCell(0).getStringCellValue().trim().equalsIgnoreCase(Constants.TCID)
//				|| !row.getCell(2).getStringCellValue().trim().equalsIgnoreCase("Description")
//				|| !row.getCell(3).getStringCellValue().trim().equalsIgnoreCase(Constants.KEYWORD)
//				|| !row.getCell(4).getStringCellValue().trim().equalsIgnoreCase(Constants.OBJECT)
//				|| !row.getCell(5).getStringCellValue().trim().equalsIgnoreCase(Constants.DATA)
//				|| !row.getCell(6).getStringCellValue().trim().equalsIgnoreCase("Proceed_on_Fail")){
//			APP_LOGS.error("Column names not defined properly in Test Steps Sheet.. Please check if there any typos");
//			return false;
//		}
		sheet = workbook.getSheetAt(0);
  ...
  // find whether sheets exists	
	...
	public static void main(String arg[]) throws IOException{
		...
		Xls_Reader datatable = null;
			 ...
			 //D:\KeyWordAutomation\config
			// datatable = new Xls_Reader("D:\\KeyWordAutomation\\config\\Controller.xlsx");
			 //datatable = new Xls_Reader(System.getProperty("user.dir")+"//src//xls//Suite.xlsx");
			 datatable=new Xls_Reader(System.getProperty("user.dir")+"//src//xls//C Suite.xlsx");
			 datatable.setCellData("SampleTest", "Result", 2, "Fail");
			 /*
				for(int col=0 ;col< datatable.getColumnCount("TC01"); col++){
					System.out.println(datatable.getCellData("TC01", col, 1));
				}
			*/	
	}
